...
import heroSchema from "../../manager/schema/heroSchema";
...
const UpdateHeroForm = () => {
  //TODO Variables
  const { theme, backendUrl, token, navigate } = useContext(UserContext);
  ...
  const handleUpdateHeroSection = async (data) => {
    const normalizeHeroForDb = normalizeHero(data);
    if (normalizeHeroForDb.title === "") {
      delete normalizeHeroForDb.title;
    }
    if (normalizeHeroForDb.subTitle === "") {
      delete normalizeHeroForDb.subTitle;
    }
    if (normalizeHeroForDb.description === "") {
      delete normalizeHeroForDb.description;
    }
    if (normalizeHeroForDb.image.url === "") {
      if (hero || hero.length !== 0) {
        normalizeHeroForDb.image.url = hero[0].image.url;
      }
    }
    if (normalizeHeroForDb.image.alt === "") {
      if (hero || hero.length !== 0) {
        normalizeHeroForDb.image.alt = hero[0].image.alt;
      }
    }
    try {
      const response = await axios.put(
        backendUrl + "/api/hero/" + heroId,
        normalizeHeroForDb,
        {
          headers: {
            "x-auth-token": token,
